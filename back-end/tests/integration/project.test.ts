...
describe("Integração - Rotas de Projetos", () => {
  ...
  it("deve listar todos os projetos de um usuário", async () => {
    const response = await request(app).get("/api/projects/user/1");
  ...
  it("deve permitir atualizar APENAS a descrição", async () => {
    const projetoCriado = await request(app).post("/api/projects").send({
        name: "Projeto Update", 
  ...
  it("deve deletar um projeto existente", async () => {
    ...
    const response = await request(app).delete(`/api/projects/${idProjeto}`);

    expect(response.status).toBe(204);

    const check = await request(app).get(`/api/projects/${idProjeto}`);
